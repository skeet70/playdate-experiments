...
use playdate::{
    ...
    graphics::{
        ...
        bitmap::{self, Bitmap},
        color::Color,
        text::{self, StringEncoding, StringEncodingExt},
        BitmapDrawMode, BitmapDrawModeExt, BitmapFlip, BitmapFlipExt,
    },
    ll_symbols,
    sound::{
        player::{Repeat, SamplePlayer},
        sample::Sample,
    },
    sys::{ffi::PlaydateAPI, EventLoopCtrl},
    system::{
        event::{SystemEvent, SystemEventExt},
        update::{Update, UpdateCtrl},
        System,
    },
};
...
impl State {
    fn new() -> Self {
        ...
        Self {
            location: Point::new(CENTER_X as _, CENTER_Y as _),
...
impl Update for State {
    ...
    fn update(&mut self) -> UpdateCtrl {
        ...
        if self.location.x < BOX_WIDTH / 2
            || self.location.x > Display::COLUMNS as i32 - BOX_WIDTH / 2
        {
            self.velocity.x = -self.velocity.x;
        }

        if self.location.y < BOX_HEIGHT / 2
            || self.location.y > Display::ROWS as i32 - BOX_HEIGHT / 2
        {
